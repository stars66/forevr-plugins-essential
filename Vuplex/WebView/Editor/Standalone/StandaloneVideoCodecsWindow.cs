...
";
        ...
        void _enableProprietaryVideoCodecsForMac() {
            ...
            var sourcePath = EditorUtils.FindDirectory(
                Path.Combine(Application.dataPath, "Vuplex/WebView/Plugins/Mac/VuplexWebViewMac_with_codecs.bundle")
            );
            var destinationPath = EditorUtils.FindDirectory(
                Path.Combine(Application.dataPath, "Vuplex/WebView/Plugins/Mac/VuplexWebViewMac.bundle")
            );
        ...
        void _enableProprietaryVideoCodecsForWindows() {
            ...
            var sourcePath = EditorUtils.FindFile(
                Path.Combine(Application.dataPath, "Vuplex/WebView/Plugins/Windows/libcef_with_codecs.dll")
            );
            var destinationPath = EditorUtils.FindFile(
                Path.Combine(Application.dataPath, "Vuplex/WebView/Plugins/Windows/VuplexWebViewChromium/libcef.dll")
            );
        ...
        void OnGUI() {
            ...
            GUILayout.Box(
                _textWithColor(DESCRIPTION_TEXT, textColor),
                new GUIStyle {
                    wordWrap = true,
                    richText = true,
            ...
            var learnMoreLinkClicked = GUILayout.Button(
                _textWithColor("Learn more", EditorUtils.GetLinkColor()),
                 new GUIStyle {
                    richText = true,
                    padding = new RectOffset {
                        top = 10,
            ...
            GUI.Label(linkRect, _textWithColor("____________", EditorUtils.GetLinkColor()), new GUIStyle {
                richText = true,
                padding = new RectOffset {
                    top = 12,
                    bottom = 15
            ...
            if (_errorMessage != null) {
                GUILayout.Label(
                    _textWithColor(_errorMessage, "red"),
                    new GUIStyle {
                        wordWrap = true,
                        richText = true,
                        padding = new RectOffset {
                ...
                GUILayout.Label(
                    String.Format("<color=green>✔</color> <color={0}>Proprietary video codecs enabled</color>", textColor),
                    new GUIStyle {
                        wordWrap = true,
                        richText = true,
                ...
                GUILayout.Label(
                    _textWithColor(TOGGLE_TEXT, textColor),
                    new GUIStyle {
                        wordWrap = true,
                        padding = new RectOffset {
                            right = 20,
        ...
        string _textWithColor(string text, string color) {
